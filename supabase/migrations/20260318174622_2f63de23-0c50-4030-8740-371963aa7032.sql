...
BEGIN
  ...
  -- Determine primary by task count
  ...
  -- Move shifts
  UPDATE shifts SET project_id = v_primary_id WHERE project_id = v_secondary_id;
  ...
  IF v_primary_scope_id IS NULL AND v_secondary_scope_id IS NOT NULL THEN
    ...
    -- Update scope's converted_project_id to point to primary
  ...
  -- Delete secondary project (members cascade)
