...
DROP POLICY IF EXISTS "Insert own shifts within 7 day window or admin" ON public.shifts;
CREATE POLICY "Insert own shifts within 7 day window or admin" ON public.shifts
FOR INSERT WITH CHECK (is_admin(auth.uid()) OR (user_id = auth.uid() AND shift_date <= public.business_today() AND shift_date >= public.business_today() - 7));

DROP POLICY IF EXISTS "Update own shifts within 7 day window or admin" ON public.shifts;
CREATE POLICY "Update own shifts within 7 day window or admin" ON public.shifts
FOR UPDATE USING (is_admin(auth.uid()) OR (user_id = auth.uid() AND shift_date <= public.business_today() AND shift_date >= public.business_today() - 7))
WITH CHECK (is_admin(auth.uid()) OR (user_id = auth.uid() AND shift_date <= public.business_today() AND shift_date >= public.business_today() - 7));

DROP POLICY IF EXISTS "Delete own shifts within 7 day window or admin" ON public.shifts;
CREATE POLICY "Delete own shifts within 7 day window or admin" ON public.shifts
FOR DELETE USING (is_admin(auth.uid()) OR (user_id = auth.uid() AND shift_date <= public.business_today() AND shift_date >= public.business_today() - 7));

DROP POLICY IF EXISTS "Insert allocations via shift access" ON public.shift_task_allocations;
CREATE POLICY "Insert allocations via shift access" ON public.shift_task_allocations
FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.shifts s WHERE s.id = shift_task_allocations.shift_id AND (is_admin(auth.uid()) OR (s.user_id = auth.uid() AND s.shift_date <= public.business_today() AND s.shift_date >= public.business_today() - 7))));

DROP POLICY IF EXISTS "Update allocations via shift access" ON public.shift_task_allocations;
CREATE POLICY "Update allocations via shift access" ON public.shift_task_allocations
FOR UPDATE USING (EXISTS (SELECT 1 FROM public.shifts s WHERE s.id = shift_task_allocations.shift_id AND (is_admin(auth.uid()) OR (s.user_id = auth.uid() AND s.shift_date <= public.business_today() AND s.shift_date >= public.business_today() - 7))));

DROP POLICY IF EXISTS "Delete allocations via shift access" ON public.shift_task_allocations;
CREATE POLICY "Delete allocations via shift access" ON public.shift_task_allocations
FOR DELETE USING (EXISTS (SELECT 1 FROM public.shifts s WHERE s.id = shift_task_allocations.shift_id AND (is_admin(auth.uid()) OR (s.user_id = auth.uid() AND s.shift_date <= public.business_today() AND s.shift_date >= public.business_today() - 7))));
...
BEGIN
  ...
  -- Non-admins: rolling window of today through 7 calendar days ago (local business day)
  IF NOT is_admin(v_caller) THEN
    IF p_shift_date > v_today THEN
      RAISE EXCEPTION 'Shift date cannot be in the future';
    END IF;
    IF p_shift_date < v_today - 7 THEN
    ...
    IF p_shift_id IS NOT NULL THEN
      ...
      END IF;
      IF v_existing_date < v_today - 7 OR v_existing_date > v_today THEN
        RAISE EXCEPTION 'Shifts older than 7 days are locked. Ask an admin to make changes.';
      END IF;
    END IF;
  END IF;
