...
BEGIN
  ...
  FOR v_step IN
    SELECT id, title, sort_order, trade, notes, assignment_mode, default_candidate_user_ids
  ...
  LOOP
    INSERT INTO public.tasks (
      ...
      COALESCE(v_step.assignment_mode, 'solo'),
      CASE WHEN v_step.assignment_mode = 'crew' AND array_length(v_step.default_candidate_user_ids, 1) > 0
           THEN v_step.default_candidate_user_ids[1]
           ELSE NULL END
    )
    ...
    -- Populate crew candidates from recipe step defaults
    IF v_step.assignment_mode = 'crew' AND array_length(v_step.default_candidate_user_ids, 1) > 0 THEN
      FOREACH v_candidate_id IN ARRAY v_step.default_candidate_user_ids LOOP
        INSERT INTO public.task_candidates (task_id, user_id)
        VALUES (v_new_task_id, v_candidate_id)
        ON CONFLICT (task_id, user_id) DO NOTHING;

        -- Auto-add as project member if not already
        INSERT INTO public.project_members (project_id, user_id, role)
        VALUES (v_parent.project_id, v_candidate_id, 'contractor')
