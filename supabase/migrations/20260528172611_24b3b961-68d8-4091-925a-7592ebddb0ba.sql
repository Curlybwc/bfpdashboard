-- 1. Drop the per-day uniqueness so workers can have multiple shifts on the same project/day
ALTER TABLE public.shifts DROP CONSTRAINT IF EXISTS shifts_user_id_project_id_shift_date_key;

-- 2. Loosen RLS on shifts: allow any past date, block future
DROP POLICY IF EXISTS "Insert own shifts within date window or admin" ON public.shifts;
CREATE POLICY "Insert own shifts past or today or admin"
  ON public.shifts FOR INSERT
  WITH CHECK (
    is_admin(auth.uid())
    OR (user_id = auth.uid() AND shift_date <= current_date)
  );

DROP POLICY IF EXISTS "Update own current-day shifts or admin" ON public.shifts;
CREATE POLICY "Update own past or today shifts or admin"
...
-- 3. Loosen RLS on shift_task_allocations the same way
  ...
  WITH CHECK (
    EXISTS (
      ...
      WHERE s.id = shift_task_allocations.shift_id
        AND (is_admin(auth.uid()) OR (s.user_id = auth.uid() AND s.shift_date <= current_date))
    )
  );
...
-- 4. Update upsert_shift_with_allocations: allow past dates, remove ON CONFLICT (uniqueness gone)
CREATE OR REPLACE FUNCTION public.upsert_shift_with_allocations(
  ...
  p_total_hours numeric DEFAULT NULL::numeric,
  p_allocations jsonb DEFAULT '[]'::jsonb,
  p_is_admin_edit boolean DEFAULT false,
  p_is_flat_rate boolean DEFAULT false,
  p_flat_rate_amount numeric DEFAULT NULL::numeric
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Block future dates for non-admins (past dates now allowed)
  IF NOT is_admin(v_caller) AND p_shift_date > current_date THEN
  ...
  ELSE
    IF p_is_flat_rate THEN
      ...
      RETURNING id INTO v_shift_id;
    ELSE
      -- Plain insert; uniqueness removed so multiple shifts per project/day are allowed
      INSERT INTO public.shifts (
