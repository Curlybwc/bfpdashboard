...
BEGIN
  ...
  LOOP
    ...
    -- Track first subtask for material migration
  ...
  -- Move any existing materials from parent task to first subtask
  IF v_first_task_id IS NOT NULL THEN
    UPDATE public.task_materials
    SET task_id = v_first_task_id
    WHERE task_id = p_parent_task_id AND is_active = true;
  END IF;
